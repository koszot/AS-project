...
geneID2GO <- readMappings(file = "umaydis_GO_dict.tsv")  

geneUniverse <- read_tsv("umaydis_enrichment/umaydis_genelvl_enrichment_geneUniverse.tsv", col_names = T, cols(value = col_character()))
geneUniverse <- as.character(geneUniverse$value)

AS.transcripts <- read_tsv("umaydis_enrichment/umaydis_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))
...
length(all_vs_AS_BP)
for (x in 1:length(all_vs_AS_BP)) {
  all_vs_AS_BP[[x]] <- str_replace(all_vs_AS_BP[[x]], "^", "umaydis_")
}
...
length(all_vs_AS_MF)
for (x in 1:length(all_vs_AS_MF)) {
  all_vs_AS_MF[[x]] <- str_replace(all_vs_AS_MF[[x]], "^", "umaydis_")
}
...
length(all_vs_AS_CC)
for (x in 1:length(all_vs_AS_CC)) {
  all_vs_AS_CC[[x]] <- str_replace(all_vs_AS_CC[[x]], "^", "umaydis_")
}
...
write_tsv(final, "umaydis_enrichment/umaydis_genelvl_enrichment_fisher_classic.tsv")
...
for (x in 1:length(all_vs_AS_BP_significant)) {
  all_vs_AS_BP_significant[[x]] <- str_replace(all_vs_AS_BP_significant[[x]], "^", "umaydis_")
}
...
for (x in 1:length(all_vs_AS_MF_significant)) {
  all_vs_AS_MF_significant[[x]] <- str_replace(all_vs_AS_MF_significant[[x]], "^", "umaydis_")
}
...
for (x in 1:length(all_vs_AS_CC_significant)) {
  all_vs_AS_CC_significant[[x]] <- str_replace(all_vs_AS_CC_significant[[x]], "^", "umaydis_")
}
...
write_tsv(final_significant, "umaydis_enrichment/umaydis_genelvl_enrichment_fisher_classic_significant.tsv")
