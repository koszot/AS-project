...
isoforms.fpkm <- read_tsv("cneoformans_CUFFdiff/isoforms.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))  # betöltjük az isoform FPKM táblát
genes.fpkm <- read_tsv("cneoformans_CUFFdiff/genes.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))           # betöltjük az genes FPKM táblát

isoforms.fpkm <- isoforms.fpkm %>%
  select(transcript_id = tracking_id, gene_id, FPKM = CN_1_FPKM)            # kiválasztjuk a minket érdeklő oszlopokat

genes.fpkm <- genes.fpkm %>%
  select(tracking_id, gene_id, FPKM = CN_1_FPKM)              # kiválasztjuk a minket érdeklő oszlopokat

stats <- read_tsv("cneoformans_stats.log")                                                 # a stats list bevezetése
...
R_value <- function(isoforms.fpkm.AS.FPKM_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    isoforms_temp$Ri <- isoforms_temp$FPKM / sum(isoforms_temp$FPKM)
...
isoforms.fpkm.AS.FPKM.Rmax <- isoforms.fpkm.AS.FPKM.Rmax[,c(1:3)]

results <- isoforms.fpkm.AS.FPKM.Rmax

rm(genes.fpkm, isoforms.fpkm, isoforms.fpkm.AS, isoforms.fpkm.AS.FPKM, isoforms.fpkm.AS.FPKM.Rmax)

#################################
#     CDS RÉGIÓK VIZSGÁLATA     #
#################################

cds <- read_tsv("cneoformans_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
CDS_length_check <- function(results.CDS_FUN){
  ...
  for (x in 1:length(ID)) {
    ...
    results_temp$is_CDS_length_same_w_primary <- results_temp[,4] == as.integer(results_temp[1,4])
    results_temp[1,5] <- NA
...
raw_interpro <- read_lines("cneoformans_proteins_all.fasta.tsv")
...
domain_same <- function(results.CDS2.DC_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$no_domain_count_change <- temp[,6] == as.integer(temp[1,6])
    temp[1,7] <- NA
...
domain_gain <- function(results.CDS2.DC.DS_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_gain <- temp[,6] > as.integer(temp[1,6])
    temp[1,8] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_loss <- function(results.CDS2.DC.DS.DG_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_loss <- temp[,6] < as.integer(temp[1,6])
    temp[1,9] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_length_compare_fix <- function(df_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    if(is.na(temp[1,7])) {
      temp[1,11] <- NA                                         # no_domain_count_change == NA, akkor primary transzkript vagyis minden NA
    } else if (temp[1,7] == F) {
      temp[1,11] <- F                                          # no_domain_count_change == FALSE, akkor történt domainszám változás vagyis vagy gain vagy loss és nem dmg
    } else if (temp[1,7] == T & temp[1,6] == 0) {
      temp[1,11] <- F                                          # no_domain_count_change == TRUE, tehát nem történt domain szám voltozás, DE domain_count == 0, vagyis nincs domain benne akkor is FALSE
    }
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
write_tsv(df3, "cneoformans_results.tsv")

write_tsv(stats, "cneoformans_stats.log")
