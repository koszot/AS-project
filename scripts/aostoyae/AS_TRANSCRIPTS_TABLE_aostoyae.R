...
stats[[1]] <- paste0("Isoforms associated /w Alternative Splicing                            Gene count: ", 
                     nrow(unique(isoforms.fpkm.AS[,"gene_id"])), 
                     "        Transcript count: ", 
                     nrow(unique(isoforms.fpkm.AS[,"transcript_id"])))            # statisztika
...
stats[[2]] <- paste0("Isoforms of genes /w more then 2 FPKM at any stage                     Gene count: ", 
                     nrow(unique(isoforms.fpkm.AS.FPKM[,"gene_id"])), 
                     "        Transcript count: ", 
                     nrow(unique(isoforms.fpkm.AS.FPKM[,"transcript_id"])))                # statisztika
...
stats[[3]] <- paste0("Isoforms w/ at least 10% contribution to overall gene expression       Gene count: ", 
                     nrow(unique(isoforms.fpkm.AS.FPKM.Rmax[,"gene_id"])), 
                     "        Transcript count: ", 
                     nrow(unique(isoforms.fpkm.AS.FPKM.Rmax[,"transcript_id"])))                # statisztika
...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_P1_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:5]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:5])            # VM to P1
    isoforms.fpkm.AS.FPKM.Rmax_FUN$P1_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,5:12]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,5:12])          # P1 to FB
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:12]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:12])          # RMA to FB
    pb <- txtProgressBar(min = 1, max = length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
  }
  isoforms.fpkm.AS.FPKM.Rmax_FUN <- isoforms.fpkm.AS.FPKM.Rmax_FUN[,c(1:12,24:26)]
...
cds <- read_tsv("aostoyae_enrichment_analysis/aostoyae_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
results.CDS$overall_FPKM <- results.CDS$RMA_FPKM + results.CDS$VM_FPKM + results.CDS$P1_FPKM + results.CDS$P2_C_FPKM + results.CDS$P2_S_FPKM + results.CDS$YFB_C_FPKM + results.CDS$YFB_S_FPKM + results.CDS$FB_C_FPKM + results.CDS$FB_L_FPKM + results.CDS$FB_S_FPKM

results.CDS <- results.CDS[,c(1:2,17,3:16)]
...
CDS_length_check <- function(results.CDS_FUN){
  ...
  for (x in 1:length(ID)) {
    ...
    results_temp$is_CDS_length_same_w_primary <- results_temp[,17] == as.integer(results_temp[1,17])
    results_temp[1,18] <- NA
...
raw_interpro <- read_lines("aostoyae_proteins_all.fasta.tsv")
...
domain_same <- function(results.CDS2.DC_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$no_domain_count_change <- temp[,19] == as.integer(temp[1,19])
    temp[1,20] <- NA
...
domain_gain <- function(results.CDS2.DC.DS_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_gain <- temp[,19] > as.integer(temp[1,19])
    temp[1,21] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_loss <- function(results.CDS2.DC.DS.DG_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_loss <- temp[,19] < as.integer(temp[1,19])
    temp[1,22] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_length_compare_fix <- function(df_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    if(is.na(temp[1,20])) {
      ...
      temp[1,24] <- F
    } else if (temp[1,20] == T & temp[1,19] == 0) {
      temp[1,24] <- F
    }
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
...
write_tsv(df3, "aostoyae_results.tsv")
