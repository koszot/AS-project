...
isoforms.fpkm <- read_tsv("pchrysosporium_CUFFdiff/isoforms.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))  # betöltjük az isoform FPKM táblát
genes.fpkm <- read_tsv("pchrysosporium_CUFFdiff/genes.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))           # betöltjük az genes FPKM táblát
...
genes.fpkm <- genes.fpkm %>%
  select(tracking_id, gene_id, VM_FPKM, YFB_FPKM, FB_FPKM)              # kiválasztjuk a minket érdeklő oszlopokat

stats <- read_tsv("pchrysosporium_stats.log")                                                 # a stats list bevezetése
...
genes_FPKM_filtering <- function(genes.fpkm_FUN, isoforms.fpkm.AS_FUN) {
  genes.fpkm.FPKM <- genes.fpkm_FUN %>% filter(VM_FPKM > 2 | 
                                                 YFB_FPKM > 2 | 
                                                 FB_FPKM > 2)                              # kiszedjük azokat a géneket amiknél az FPKM érték nagyobb mint 2 bármelyik fejlődési szakaszban
  ...
  isoforms.fpkm.AS.FPKM <- isoforms.fpkm.AS.FPKM %>%
    select(gene_id, transcript_id, VM_FPKM, YFB_FPKM, FB_FPKM) 
...
R_value <- function(isoforms.fpkm.AS.FPKM_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    isoforms_temp$FB_Ri <- isoforms_temp$FB_FPKM / sum(isoforms_temp$FB_FPKM)
...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5])
...
isoforms.fpkm.AS.FPKM.Rmax.FC <- isoforms.fpkm.AS.FPKM.Rmax.FC[,c(1:5,10)]
...
cds <- read_tsv("pchrysosporium_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
results.CDS$overall_FPKM <- results.CDS$VM_FPKM + results.CDS$YFB_FPKM + results.CDS$FB_FPKM

results.CDS <- results.CDS[,c(1:2,8,3:7)]
...
CDS_length_check <- function(results.CDS_FUN){
  ...
  for (x in 1:length(ID)) {
    ...
    results_temp$is_CDS_length_same_w_primary <- results_temp[,8] == as.integer(results_temp[1,8])
    results_temp[1,9] <- NA
...
raw_interpro <- read_lines("pchrysosporium_proteins_all.fasta.tsv")
...
domain_same <- function(results.CDS2.DC_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$no_domain_count_change <- temp[,10] == as.integer(temp[1,10])
    temp[1,11] <- NA
...
domain_gain <- function(results.CDS2.DC.DS_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_gain <- temp[,10] > as.integer(temp[1,10])
    temp[1,12] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_loss <- function(results.CDS2.DC.DS.DG_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_loss <- temp[,10] < as.integer(temp[1,10])
    temp[1,13] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_length_compare_fix <- function(df_FUN) {
  mydata <- data.frame()
  ID <- unique(df_FUN$transcript_id)
  for (x in 1:length(ID)) {
    temp <- df_FUN[df_FUN$transcript_id %in% ID[x],]
    if(is.na(temp[1,11])) {
      temp[1,15] <- NA                                         # no_domain_count_change == NA, akkor primary transzkript vagyis minden NA
    } else if (temp[1,11] == F) {
      temp[1,15] <- F                                          # no_domain_count_change == FALSE, akkor történt domainszám változás vagyis vagy gain vagy loss és nem dmg
    } else if (temp[1,11] == T & temp[1,10] == 0) {
      temp[1,15] <- F                                          # no_domain_count_change == TRUE, tehát nem történt domain szám voltozás, DE domain_count == 0, vagyis nincs domain benne akkor is FALSE
    }
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
write_tsv(df3, "pchrysosporium_results.tsv")

write_tsv(stats, "pchrysosporium_stats.log")
