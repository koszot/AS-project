...
isoforms.fpkm <- read_tsv("aampla_CUFFdiff/isoforms.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))  # betöltjük az isoform FPKM táblát
genes.fpkm <- read_tsv("aampla_CUFFdiff/genes.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))           # betöltjük az genes FPKM táblát
...
genes.fpkm <- genes.fpkm %>%
  select(tracking_id, gene_id, VM_FPKM, P1_FPKM, P2_FPKM, YFB_FPKM, FB_FPKM)              # kiválasztjuk a minket érdeklő oszlopokat

stats <- read_tsv("aampla_stats.log")                                                 # a stats list bevezetése
...
cds <- read_tsv("aampla_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
raw_interpro <- read_lines("aampla_proteins_all.fasta.tsv")
...
write_tsv(df3, "aampla_results.tsv")
