...
original <- read_tsv("umaydis_genome/original.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand",  "att2", "attributes"))
...
write.table(original_fixed, file = "umaydis_genome/umaydis_onlyexon.gtf", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write.table(genes, file = "./umaydis_genome/umaydis_onlygene.gtf", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
exon_fix <- function(original_forFUN) 
{
  ...
  exon_fixed$chr <- exon_fixed$tid_id
...
write.table(exon_fixed, file = "./umaydis_genome/umaydis_fixed.gtf", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
