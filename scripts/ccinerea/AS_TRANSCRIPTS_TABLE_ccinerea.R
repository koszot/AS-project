...
stats <- read_tsv("ccinerea_AmutBmut_stats.log")                                                 # a stats list bevezetése
...
stats[6,1] <- "Isoforms associated /w Alternative Splicing"
stats[6,2] <- nrow(unique(isoforms.fpkm.AS[,"gene_id"]))
stats[6,3] <- nrow(unique(isoforms.fpkm.AS[,"transcript_id"]))
...
stats[7,1] <- "Isoforms of genes /w more then 2 FPKM at any stage"
stats[7,2] <- nrow(unique(isoforms.fpkm.AS.FPKM[,"gene_id"]))
stats[7,3] <- nrow(unique(isoforms.fpkm.AS.FPKM[,"transcript_id"]))
...
R_value <- function(isoforms.fpkm.AS.FPKM_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    isoforms_temp$H_Ri <- isoforms_temp$H_FPKM / sum(isoforms_temp$H_FPKM)
    isoforms_temp$P1_Ri <- isoforms_temp$P1_FPKM / sum(isoforms_temp$P1_FPKM)
    isoforms_temp$P2_Ri <- isoforms_temp$P2_FPKM / sum(isoforms_temp$P2_FPKM)
    isoforms_temp$YFB_K_Ri <- isoforms_temp$YFB_K_FPKM / sum(isoforms_temp$YFB_K_FPKM)
    isoforms_temp$YFB_L_Ri <- isoforms_temp$YFB_L_FPKM / sum(isoforms_temp$YFB_L_FPKM)
    isoforms_temp$YFB_T_Ri <- isoforms_temp$YFB_T_FPKM / sum(isoforms_temp$YFB_T_FPKM)
    isoforms_temp$FB_KL_Ri <- isoforms_temp$FB_KL_FPKM / sum(isoforms_temp$FB_KL_FPKM)
...
stats[8,1] <- "Isoforms w/ at least 10% contribution to overall gene expression"
stats[8,2] <- nrow(unique(isoforms.fpkm.AS.FPKM.Rmax[,"gene_id"]))
stats[8,3] <- nrow(unique(isoforms.fpkm.AS.FPKM.Rmax[,"transcript_id"]))
...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_P1_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5])
    isoforms.fpkm.AS.FPKM.Rmax_FUN$P1_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,5:11]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,5:11])
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:11]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:11])
...
isoforms.fpkm.AS.FPKM.Rmax.FC <- isoforms.fpkm.AS.FPKM.Rmax.FC[,c(1:11,22:24)]
...
cds <- read_tsv("ccinerea_AmutBmut_enrichment_analysis/ccinerea_AmutBmut_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
results.CDS$overall_FPKM <- results.CDS$VM_FPKM + results.CDS$H_FPKM + results.CDS$P1_FPKM + results.CDS$P2_FPKM + results.CDS$YFB_K_FPKM + results.CDS$YFB_L_FPKM + results.CDS$YFB_T_FPKM + results.CDS$FB_KL_FPKM + results.CDS$FB_T_FPKM

results.CDS <- results.CDS[,c(1:2,16,3:15)]
...
CDS_length_check <- function(results.CDS_FUN){
  ...
  for (x in 1:length(ID)) {
    ...
    results_temp$is_CDS_length_same_w_primary <- results_temp[,16] == as.integer(results_temp[1,16])
    results_temp[1,17] <- NA
...
raw_interpro <- read_lines("ccinerea_AmutBmut_proteins_all.fasta.tsv")
...
domain_same <- function(results.CDS2.DC_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$no_domain_count_change <- temp[,18] == as.integer(temp[1,18])
    temp[1,19] <- NA
...
domain_gain <- function(results.CDS2.DC.DS_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_gain <- temp[,18] > as.integer(temp[1,18])
    temp[1,20] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_loss <- function(results.CDS2.DC.DS.DG_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    temp$is_domain_loss <- temp[,18] < as.integer(temp[1,18])
    temp[1,21] <- NA
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
domain_length_compare_fix <- function(df_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    if(is.na(temp[1,19])) {
      temp[1,23] <- NA                                         # no_domain_count_change == NA, akkor primary transzkript vagyis minden NA
    } else if (temp[1,19] == F) {
      temp[1,23] <- F                                          # no_domain_count_change == FALSE, akkor történt domainszám változás vagyis vagy gain vagy loss és nem dmg
    } else if (temp[1,19] == T & temp[1,18] == 0) {
      temp[1,23] <- F                                          # no_domain_count_change == TRUE, tehát nem történt domain szám voltozás, DE domain_count == 0, vagyis nincs domain benne akkor is FALSE
    }
    mydata <- rbind(mydata, temp)
    pb <- txtProgressBar(min = 1, max = length(ID), style = 3)                      # progress bar
    setTxtProgressBar(pb, x, title = NULL, label = NULL)
...
write_tsv(df3, "ccinerea_AmutBmut_results.tsv")

write_tsv(stats, "ccinerea_AmutBmut_stats.log")
