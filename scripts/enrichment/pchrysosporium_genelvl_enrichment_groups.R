...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:5])            # VM to FB
...
write_tsv(geneUniverse, "pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_geneUniverse.tsv")

AS.transcripts <- as_tibble(unique(AS.transcripts$gene_id))
write_tsv(AS.transcripts, "pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_AStranscripts.tsv")

DEVREG.transcripts <- as_tibble(unique(DEVREG.transcripts$gene_id))
write_tsv(DEVREG.transcripts, "pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_DEVREGtranscripts.tsv")
