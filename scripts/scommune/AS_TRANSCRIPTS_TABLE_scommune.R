...
isoforms.fpkm <- read_tsv("scommune_CUFFdiff/isoforms.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))  # betöltjük az isoform FPKM táblát
genes.fpkm <- read_tsv("scommune_CUFFdiff/genes.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))           # betöltjük az genes FPKM táblát

isoforms.fpkm <- isoforms.fpkm %>%
  select(transcript_id = tracking_id, gene_id, VM_FPKM, 
         P1_FPKM, P2_FPKM, YFB_FPKM, FB_FPKM)            # kiválasztjuk a minket érdeklő oszlopokat

genes.fpkm <- genes.fpkm %>%
  select(tracking_id, gene_id, VM_FPKM, 
         P1_FPKM, P2_FPKM, YFB_FPKM, FB_FPKM)              # kiválasztjuk a minket érdeklő oszlopokat

stats <- read_tsv("scommune_stats.log")                                                 # a stats list bevezetése
...
genes_FPKM_filtering <- function(genes.fpkm_FUN, isoforms.fpkm.AS_FUN) {
  ...
  isoforms.fpkm.AS.FPKM <- isoforms.fpkm.AS.FPKM %>%
    select(gene_id, transcript_id, VM_FPKM, P1_FPKM, P2_FPKM, YFB_FPKM, FB_FPKM) 
...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_P1_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:4]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:4])            # VM to P1
    isoforms.fpkm.AS.FPKM.Rmax_FUN$P1_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:7]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:7])          # P1 to FB
    isoforms.fpkm.AS.FPKM.Rmax_FUN$VM_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:7]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,3:7])          # VM to FB
...
cds <- read_tsv("scommune_enrichment_analysis/scommune_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
results.CDS$overall_FPKM <- results.CDS$VM_FPKM + results.CDS$P1_FPKM + results.CDS$P2_FPKM + results.CDS$YFB_FPKM + results.CDS$FB_FPKM
...
raw_interpro <- read_lines("scommune_proteins_all.fasta.tsv")
...
write_tsv(df3, "scommune_results.tsv")

write_tsv(stats, "scommune_stats.log")
