...
setwd("~/Desktop/alternative_splicing/rmellea/")  # beállítjuk a working directoryt

genes <- read_tsv("rmellea_genome/rmellea_onlygene.gtf", 
                  col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))     # beolvassuk az eredeti GTF fájlt

raw <- read_tsv("rmellea_RRPM_CUFFout/transcripts.gtf", 
...
write.table(strand_filtered_final, file = "./rmellea_genome/rmellea_RRPM_transcripts.gtf", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
...
original <- read_tsv("rmellea_genome/rmellea_onlyexon.gtf", 
                  col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))     # beolvassuk az eredeti GTF fájlt
...
write.table(merged, file = "./rmellea_genome/rmellea_AS_annotation.gtf", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
