...
isoforms.fpkm <- read_tsv("ltigrinus_CUFFdiff/isoforms.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))  # betöltjük az isoform FPKM táblát
genes.fpkm <- read_tsv("ltigrinus_CUFFdiff/genes.fpkm_tracking", col_names = T, cols(.default = col_guess(), tracking_id = col_character(), gene_id = col_character()))           # betöltjük az genes FPKM táblát
...
genes.fpkm <- genes.fpkm %>%
  select(tracking_id, gene_id, VM_FPKM, P1_FPKM, P2_K_FPKM, P2_T_FPKM, YFB_K_FPKM, YFB_T_FPKM, FB_K_FPKM, FB_L_FPKM, FB_T_FPKM)              # kiválasztjuk a minket érdeklő oszlopokat

stats <- read_tsv("ltigrinus_stats.log")                                                 # a stats list bevezetése
...
genes_FPKM_filtering <- function(genes.fpkm_FUN, isoforms.fpkm.AS_FUN) {
  ...
  isoforms.fpkm.AS.FPKM <- isoforms.fpkm.AS.FPKM %>%
    select(gene_id, transcript_id, VM_FPKM, P1_FPKM, P2_K_FPKM, P2_T_FPKM, YFB_K_FPKM, YFB_T_FPKM, FB_K_FPKM, FB_L_FPKM, FB_T_FPKM) 
...
R_value <- function(isoforms.fpkm.AS.FPKM_FUN) {
  ...
  for (x in 1:length(ID)) {
    ...
    isoforms_temp$P2_K_Ri <- isoforms_temp$P2_K_FPKM / sum(isoforms_temp$P2_K_FPKM)
    isoforms_temp$P2_T_Ri <- isoforms_temp$P2_T_FPKM / sum(isoforms_temp$P2_T_FPKM)
...
fold_change <- function(isoforms.fpkm.AS.FPKM.Rmax_FUN) {
  for (x in 1:length(isoforms.fpkm.AS.FPKM.Rmax_FUN$transcript_id)) {
    ...
    isoforms.fpkm.AS.FPKM.Rmax_FUN$P1_to_FB_FoldChange[x] <- max(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:11]) / min(isoforms.fpkm.AS.FPKM.Rmax_FUN[x,4:11])
...
cds <- read_tsv("ltigrinus_enrichment_analysis/ltigrinus_transcripts.fasta.transdecoder_dir/headers.cds", col_names = F)
...
results.CDS$overall_FPKM <- results.CDS$VM_FPKM + results.CDS$P1_FPKM + results.CDS$P2_K_FPKM + results.CDS$P2_T_FPKM + results.CDS$YFB_K_FPKM + results.CDS$YFB_T_FPKM + results.CDS$FB_K_FPKM + results.CDS$FB_L_FPKM + results.CDS$FB_T_FPKM
...
raw_interpro <- read_lines("ltigrinus_proteins_all.fasta.tsv")
...
write_tsv(df3, "ltigrinus_results.tsv")
