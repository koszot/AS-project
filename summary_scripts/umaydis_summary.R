...
genecatalog <- read_tsv("umaydis_genome/umaydis_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("umaydis_genome/umaydis_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("umaydis_genome/umaydis_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("umaydis_genome/umaydis_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("umaydis_ASpli_binFeatures.log")

AS <- read_tsv("umaydis_enrichment/umaydis_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("p3_t237631_Ust_maydi_v2GB.prot.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_umaydis", "%_umaydis")
...
enrichment <- read_tsv("umaydis_enrichment/umaydis_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_umaydis` <- round(summary$`%_umaydis`, digits = 3)

write_tsv(summary, "umaydis_summary.tsv")
