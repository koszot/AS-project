...
genecatalog <- read_tsv("scommune_genome/original_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("scommune_genome/original_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("scommune_genome/scommune_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("scommune_genome/scommune_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("scommune_ASpli_binFeatures.log")

AS <- read_tsv("scommune_enrichment/scommune_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

DEVREG <- read_tsv("scommune_enrichment/scommune_genelvl_enrichment_DEVREGtranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Schco3_GeneCatalog_proteins_20130812.aa.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_scommune", "%_scommune")
...
enrichment <- read_tsv("scommune_enrichment/scommune_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_scommune` <- round(summary$`%_scommune`, digits = 3)

write_tsv(summary, "scommune_summary.tsv")
