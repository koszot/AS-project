...
genecatalog <- read_tsv("pchrysosporium_genome/pchrysosporium_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("pchrysosporium_genome/pchrysosporium_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("pchrysosporium_genome/pchrysosporium_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("pchrysosporium_genome/pchrysosporium_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("pchrysosporium_ASpli_binFeatures.log")

AS <- read_tsv("pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

DEVREG <- read_tsv("pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_DEVREGtranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Phchr2_GeneCatalog_proteins_20131210.aa.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_pchrysosporium", "%_pchrysosporium")
...
enrichment <- read_tsv("pchrysosporium_enrichment/pchrysosporium_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_pchrysosporium` <- round(summary$`%_pchrysosporium`, digits = 3)

write_tsv(summary, "pchrysosporium_summary.tsv")
