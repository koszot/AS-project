...
genecatalog <- read_tsv("aostoyae_genome/original_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("aostoyae_genome/original_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("aostoyae_genome/aostoyae_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("aostoyae_genome/aostoyae_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("aostoyae_ASpli_binFeatures.log")

AS <- read_tsv("aostoyae_enrichment/aostoyae_genelvl_enrichment_AStranscripts.tsv")

DEVREG <- read_tsv("aostoyae_enrichment/aostoyae_genelvl_enrichment_DEVREGtranscripts.tsv")

genecatalogIPR <- read_lines("p3_i2_t47428_Arm_ostoy_v2.prot.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_aostoyae", "%_aostoyae")
...
enrichment <- read_tsv("aostoyae_enrichment/aostoyae_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_aostoyae` <- round(summary$`%_aostoyae`, digits = 3)

write_tsv(summary, "aostoyae_summary.tsv")
