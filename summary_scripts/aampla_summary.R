...
genecatalog <- read_tsv("aampla_genome/aampla_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("aampla_genome/aampla_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("aampla_genome/aampla_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("aampla_genome/aampla_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("aampla_ASpli_binFeatures.log")

AS <- read_tsv("aampla_enrichment/aampla_genelvl_enrichment_AStranscripts.tsv")

DEVREG <- read_tsv("aampla_enrichment/aampla_genelvl_enrichment_DEVREGtranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Auramp1_GeneCatalog_proteins_20160719.aa.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_aampla", "%_aampla")
...
enrichment <- read_tsv("aampla_enrichment/aampla_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_aampla` <- round(summary$`%_aampla`, digits = 3)

write_tsv(summary, "aampla_summary.tsv")
