...
genecatalog <- read_tsv("rmellea_genome/rmellea_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("rmellea_genome/rmellea_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("rmellea_genome/rmellea_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("rmellea_genome/rmellea_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("rmellea_ASpli_binFeatures.log")

AS <- read_tsv("rmellea_enrichment/rmellea_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

DEVREG <- read_tsv("rmellea_enrichment/rmellea_genelvl_enrichment_DEVREGtranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Ricmel1_GeneCatalog_proteins_20151108.aa.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_rmellea", "%_rmellea")
...
enrichment <- read_tsv("rmellea_enrichment/rmellea_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_rmellea` <- round(summary$`%_rmellea`, digits = 3)

write_tsv(summary, "rmellea_summary.tsv")
