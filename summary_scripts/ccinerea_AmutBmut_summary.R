...
genecatalog <- read_tsv("ccinerea_AmutBmut_genome/ccinerea_AmutBmut_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("ccinerea_AmutBmut_genome/ccinerea_AmutBmut_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("ccinerea_AmutBmut_genome/ccinerea_AmutBmut_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("ccinerea_AmutBmut_genome/ccinerea_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("ccinerea_AmutBmut_ASpli_binFeatures.log")

AS <- read_tsv("ccinerea_AmutBmut_enrichment/ccinerea_AmutBmut_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

DEVREG <- read_tsv("ccinerea_AmutBmut_enrichment/ccinerea_AmutBmut_genelvl_enrichment_DEVREGtranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Copci_AmutBmut1_GeneCatalog_proteins_20130522.aa.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_ccinerea_AmutBmut", "%_ccinerea_AmutBmut")
...
enrichment <- read_tsv("ccinerea_AmutBmut_enrichment/ccinerea_AmutBmut_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_ccinerea_AmutBmut` <- round(summary$`%_ccinerea_AmutBmut`, digits = 3)

write_tsv(summary, "ccinerea_AmutBmut_summary.tsv")
