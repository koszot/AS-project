...
genecatalog <- read_tsv("cneoformans_genome/cneoformans_onlygene.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

genecatalog_exons <- read_tsv("cneoformans_genome/cneoformans_onlyexon.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ") %>%
  select(transcriptID,geneID)

AS.annotation <- read_tsv("cneoformans_genome/cneoformans_AS_annotation.gtf",  col_names = c("chr", "maker","type", "start", "end", "att1", "strand", "att2", "attributes")) %>%
  select(attributes) %>%
  separate(attributes, c("transcriptID_label", "transcriptID",
                         "geneID_label", "geneID"), sep = " ")

RRPM_transcripts <- read_tsv("cneoformans_genome/cneoformans_RRPM_transcripts.gtf", col_names = c("chr", "maker", "type", "start", "end", "att1", "strand", "att2", "attributes"))

AS_count <- read_lines("cneoformans_ASpli_binFeatures.log")

AS <- read_tsv("cneoformans_enrichment/cneoformans_genelvl_enrichment_AStranscripts.tsv", col_names = T, cols(value = col_character()))

genecatalogIPR <- read_lines("Cryptococcus_neoformans_H99.proteins.fasta.tsv")
...
summary[1:2,3] <- 0
colnames(summary) <- c("description", "N_cneoformans", "%_cneoformans")
...
summary[13,1] <- "Expressed genes /w 7 isoforms"
summary[13,2] <- isoformcount %>% filter(V2 == 7) %>% nrow()
...
summary[13,3] <- summary[12,2]/(summary[5,2]/100)
...
summary[14,1] <- "Alternative Splicing ES bins"
summary[14,2] <- AS_count[1,2]
summary[15,1] <- "Alternative Splicing IR bins"
summary[15,2] <- AS_count[2,2]
summary[16,1] <- "Alternative Splicing Alt5SS bins"
summary[16,2] <- AS_count[3,2]
summary[17,1] <- "Alternative Splicing Alt3SS bins"
summary[17,2] <- AS_count[4,2]
summary[14,3] <- summary[14,2]/(sum(summary[14:17,2])/100)
summary[15,3] <- summary[15,2]/(sum(summary[14:17,2])/100)
summary[16,3] <- summary[16,2]/(sum(summary[14:17,2])/100)
summary[17,3] <- summary[17,2]/(sum(summary[14:17,2])/100)

#### AS ####

summary[18,1] <- "Genes /w 2 FPKM and Alternative Splicing"
summary[18,2] <- AS %>% nrow()
...
enrichment <- read_tsv("cneoformans_enrichment/cneoformans_genelvl_enrichment_fisher_classic.tsv")
...
summary$`%_cneoformans` <- round(summary$`%_cneoformans`, digits = 3)

write_tsv(summary, "cneoformans_summary.tsv")
